...
Next, add the following line at the beginning of the <!-- <if language="js"> -->`src/frontend.js`<!-- </if> --><!-- <if language="ts"> -->`src/frontend.tsx`<!-- </if> --> file to import the [`Routable()`](https://layrjs.com/docs/v2/reference/routable) mixin and the [`@route()`](https://layrjs.com/docs/v2/reference/routable#route-decorator) decorator:
...
Now that the `Guestbook` component is routable, we can use the [`@route()`](https://layrjs.com/docs/v2/reference/routable#route-decorator) decorator to associate a URL to any of its class methods.
