...
import {MemoryRouter, isRouter} from '../../..';

describe('MemoryRouter', () => {
  test('new MemoryRouter()', async () => {
  ...
  describe('History and navigation', () => {
    ...
    const getRouter = function() {
      ...
      class Movie extends Routable(Component) {
        ...
        @route('/movies/:id/about') static About({id}) {
          return `About movie #${id}`;
        }
      }

      const router = new MemoryRouter([Home, MovieList, Movie], {
        initialURLs: ['/', '/movies', '/movies/abc123']
      });
    ...
    test('getCurrentURL()', async () => {
      ...
      expect(router.getCurrentURL()).toBe('/movies/abc123');
    });
