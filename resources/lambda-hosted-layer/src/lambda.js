    ...
    if (!this._zipArchive) {
      await task(
        async () => {
          ...
          const layerCode = `module.exports = require('./${relativeMainFile}');\n`;
          const layerFile = join(tempDirectory, 'layer.js');
          writeFileSync(layerFile, layerCode);
          const {atime, mtime} = statSync(mainFile);
          utimesSync(layerFile, atime, mtime);
          allFiles.push('layer.js');
